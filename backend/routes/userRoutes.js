...
// GET /api/users — Trae todos los usuarios con sus permisos si son admin
router.get('/', async (req, res) => {
  try {
    // Traemos todos los usuarios
    const users = await User.find({}, '_id name email isAdmin');

    // Creamos un array de sus _id
    const userIds = users.map(user => user._id);

    // Buscamos todos los documentos Admin asociados
    const adminDocs = await Admin.find({ user: { $in: userIds } });

    // Creamos un mapa rápido {userId: permisos[]}
    const permisosMap = {};
    adminDocs.forEach(doc => {
      permisosMap[doc.user.toString()] = doc.permisos;
    });

    // Componemos el resultado final con los permisos solo si isAdmin === true
    const usersWithPermisos = users.map(user => ({
      ...
      permissions: user.isAdmin
        ? (permisosMap[user._id.toString()] || [])
        : []
    }));

    return res.status(200).json(usersWithPermisos);
